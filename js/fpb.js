(() => {
  ...
  function useFactorPair(button) {
    ...
    if (drop.querySelector(':scope > span')) drop.innerHTML = '';
    const pair = document.createElement('div');
    pair.className = 'revealed-factor-pair';
    pair.innerHTML = `<span>${left}</span><i aria-hidden="true">→</i><span>${right}</span><small>${left} × ${right} = ${group.replace('a2-', '')}</small>`;
  ...
  function updatePairStages(group) {
    ...
    if (['a2-24', 'a2-36'].includes(group) && groupComplete('a2-24') && groupComplete('a2-36')) {
      $('a2FpbChoiceStage')?.classList.remove('muted-stage');
  ...
  function renderMoveObjects() {
    ...
    if (!source) return;
    source.innerHTML = '<strong class="source-label">Benda:</strong>' + Array.from({length: 4}, (_, i) => objectMarkup('pencil', `p${i + 1}`)).join('') + Array.from({length: 6}, (_, i) => objectMarkup('eraser', `e${i + 1}`)).join('');
    document.querySelectorAll('#movePackageGrid .bin-objects').forEach(el => el.innerHTML = '');
    selectedObject = null;
    updateObjectBins();
  }
  ...
  function refreshA2TreeStage() {
    ...
    $('a2PairStage')?.classList.toggle('muted-stage', !ready);
    $('a2PairStage')?.classList.toggle('ready-stage', ready);
    if (ready && !$('a2PairStage')?.dataset.announced) {
      $('a2PairStage').dataset.announced = '1';
      Navigation?.toast?.('Pohon faktor selesai. Lanjutkan dengan pasangan faktor.');
    }
  }

  function validateA2FpbChoice() {
    if (!groupComplete('a2-24') || !groupComplete('a2-36')) return;
    const value = Number(ChoiceUI.selectedValue('a2fpb2436'));
    if (!value) return;
    const ok = value === 12;
  ...
  function bindPairInteractions() {
    document.addEventListener('click', e => {
      const source = e.target.closest('.factor-pair-source');
      if (source) useFactorPair(source);
    });
    ...
    document.addEventListener('dragend', e => {
      e.target.closest('.factor-pair-source')?.classList.remove('dragging');
      draggedPair = null;
      document.querySelectorAll('.factor-pair-drop').forEach(z => z.classList.remove('drag-over'));
    });
    document.addEventListener('dragover', e => {
      const zone = e.target.closest('.factor-pair-drop');
      if (!zone || !draggedPair || zone.dataset.pairDrop !== draggedPair.dataset.pairGroup) return;
      e.preventDefault();
  ...
  function bindObjectInteractions() {
    document.addEventListener('click', e => {
      const obj = e.target.closest('.move-object');
      if (obj) {
        selectMoveObject(obj);
        return;
      }
      const bin = e.target.closest('.move-package-bin');
      if (bin && selectedObject) moveObjectToBin(selectedObject, bin);
    });
    document.addEventListener('dragstart', e => {
      const obj = e.target.closest('.move-object');
      if (!obj) return;
    ...
    document.addEventListener('dragend', e => {
      e.target.closest('.move-object')?.classList.remove('dragging');
      draggedObject = null;
      document.querySelectorAll('.move-package-bin').forEach(b => b.classList.remove('drag-over'));
    });
    document.addEventListener('dragover', e => {
      const bin = e.target.closest('.move-package-bin');
      if (!bin || !draggedObject) return;
      e.preventDefault();
      bin.classList.add('drag-over');
    });
    document.addEventListener('dragleave', e => e.target.closest('.move-package-bin')?.classList.remove('drag-over'));
    document.addEventListener('drop', e => {
      const bin = e.target.closest('.move-package-bin');
      if (!bin || !draggedObject) return;
      e.preventDefault();
